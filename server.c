...
int recursive_search(char *dir_name, char **file_types, int extension_count, char *files, int *files_len)
{
    ...
    //If the entry is a subdirectory (indicated by the DT_DIR flag in the d_type field of the dirent structure), 
	//the function recursively calls itself with the subdirectory as the new dir_name parameter.
	while ((ent = readdir(dir)) != NULL)
	{
		...
		else
		{
			...
			{
				if (fnmatch(file_types[i], ent->d_name, 0) == 0) // matching the extension of the file using fnmatch
				{
...
int gettargz(int client_sockfd, char **extensions, int extension_count, int unzip)
{
	...
	//creates an array of file types by concatenating each extension with the wildcard character *, 
	//and passes this array along with the number of file types to the recursive_search function
	for (i = 0; i < extension_count; i++)
	{
		file_types[i] = malloc(strlen(extensions[i]) + 2);
		sprintf(file_types[i], "*.%s", extensions[i]);
	}
	 //searches recursively through the home directory for files with the given extensions and returns a 
	 //list of the absolute file paths of all matching files in the files array.
	int *files_len = 0;
	//If recursive_search fails to find any files, the function sends an error message to the client and returns. 
	...
	// after recursive search,the function replaces any spaces in the file paths with escape 
	//characters and concatenates all the file paths into a single string
	replace(files);
	printf("Files: %s\n", files);
	//contsructs a tar with 'files'
	sprintf(buffer, tar_command_format, temp_tar_name, files);
	printf("%s\n", buffer);
	//system function is called to execute tar command
...
// funtion to exectute getfiles command
...
void sgetfiles(int client_sockfd, char **arguments)
{
	...
	if (result == 0)
	{
		...
		fseek(fp, 0, SEEK_END);//pointer at the end 
		file_size = ftell(fp); // using the pointer at end calculate file size
...
int main(int argc, char *argv[])
{
	...
	{ 
		...
		//When a client connects to the server, the accept() function is called 
		//to accept the connection and create a new socket descriptor csd.
